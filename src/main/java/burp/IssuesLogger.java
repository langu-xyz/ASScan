 ...
 */
public class IssuesLogger implements IExtensionStateListener {
     ...
     */
    IssuesLogger(String storeName, IBurpExtenderCallbacks callbacks, Trace trace) throws Exception {
    ...
    void issuesEvent(VulnIssue vulnIssue) throws Exception {
        this.ensureDBState();

        try (PreparedStatement stmt = this.storageConnection.prepareStatement(SQL_ISSUES_TABLE_INSERT)) {
            stmt.setString(1, vulnIssue.getHttpService().toString());
            stmt.setString(2, vulnIssue.getUrl().toString());
            stmt.setString(3, vulnIssue.getHttpMessages().toString());
            stmt.setString(4, vulnIssue.getIssueName());
            stmt.setString(5, vulnIssue.getIssueDetail());
            stmt.setString(6, vulnIssue.getSeverity());
            stmt.executeUpdate();
        }
    }
